...
@interface EnterPhoneController ()

@end

@implementation EnterPhoneController
{
...
- (void)initViews
{
    _countryCodeTF.layer.cornerRadius = 10.0f;
    _phoneTF.layer.cornerRadius = 10.0f;
    
    _whyView.layer.cornerRadius = 10.0f;
    _okLabel.layer.cornerRadius = 10.0f;
    _okLabel.clipsToBounds = YES;
    
    currentEditField = _phoneTF;
}

#pragma mark - Actions

- (void)askForConfirmationCode
{
    ServerHandler *server = [[ServerHandler alloc]initWithDelegate:self];
    [server createUserConfimationCodeWithPhoneNumber:_phoneTF.text];
}

- (void)goToVerifiedCodeContorller
{
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Main" bundle: nil];
    VerifiedCodeController *verifiedCodeController = [storyboard instantiateViewControllerWithIdentifier:@"VerifiedCodeController"];
    verifiedCodeController.phoneNumber = [NSString stringWithFormat:@"%@%@", _countryCodeTF.text, _phoneTF.text];
    [self.navigationController pushViewController:verifiedCodeController animated:YES];
}
...
#pragma mark - Server Deleagte

- (void)didGetCodeFromServer
{
    [self goToVerifiedCodeContorller];
}
...
- (IBAction)didPressDial:(CircleButtonWithBorder *)sender
{
    ...
    NSString *currentPhoneText = currentEditField.text;
    
    if (text==nil) {
        if ([currentPhoneText isEqualToString:@"+"])
        {
            return;
        }
        NSString *oneDigitDeleted =currentPhoneText.length==0 ? @"": [currentPhoneText substringToIndex:currentPhoneText.length - 1];
        currentEditField.text = oneDigitDeleted;
        return;
    }
    if ([text isEqualToString:@"ok"]) {
        _phoneTF.text = [_phoneTF.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
//        [self goToVerifiedCodeContorller]; // For testing
        ServerHandler *server = [[ServerHandler alloc]initWithDelegate:self];
        if ([_countryCodeTF.text isEqualToString:@""]) {
            _countryCodeTF.text = @"+972";
        }
        NSString *phoneNumber = [NSString stringWithFormat:@"%@%@", _countryCodeTF.text, _phoneTF.text];
        [[NSUserDefaults standardUserDefaults]setObject:phoneNumber forKey:@"MyPhoneNumber"];
        [[NSUserDefaults standardUserDefaults]synchronize];
        [server createUserConfimationCodeWithPhoneNumber:[NSString stringWithFormat:@"%@%@", _countryCodeTF.text, _phoneTF.text]];
        return;
    }
    currentEditField.text = [currentPhoneText stringByAppendingString:text];
}
...
{
    textField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:PHONE_PLACE_HOLDER attributes:@{NSForegroundColorAttributeName : [UIColor whiteColor]}];
}
    ...
    [reverseGeocoder reverseGeocodeLocation:currentLocation completionHandler:^(NSArray *placemarks, NSError *error)
     {
         ...
         NSObject *countryDialCode = dictDialingCodes[myPlacemark.ISOcountryCode];
         _countryCodeTF.text = [NSString stringWithFormat:@"+%@", countryDialCode!=nil?countryDialCode:@""];
     }];
}

-(void)setCountryCodesDict{
    dictDialingCodes = @{@"IL" : @"972",
                         @"AF" : @"93",
                         @"AL" : @"355",
                         @"DZ" : @"213",
                         @"AS" : @"1",
                         ...
                         @"BG" : @"359", @"BF" : @"226", @"BI" : @"257", @"KH" : @"855",
                         @"CM" : @"237", @"CA" : @"1", @"CV" : @"238", @"KY" : @"345",
